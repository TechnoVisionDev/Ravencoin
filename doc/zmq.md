...
The Gemma Core daemon can be configured to act as a trusted "border
router", implementing the gemma wire protocol and relay, making
...
The ZeroMQ feature in Gemma Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building gemmad:
...
For instance:
    ...
    $ gemmad -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/gemmad.tx.raw
...
These options can also be provided in gemma.conf.
...
From the perspective of gemmad, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into gemmad directly. Furthermore, no information is
...
using. Gemmad appends an up-counting sequence number to each
