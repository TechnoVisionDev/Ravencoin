...
dnl Copyright (c) 2021 The Gemma Core developers
dnl Distributed under the MIT software license, see the accompanying
dnl file COPYING or http://www.opensource.org/licenses/mit-license.php.

dnl Helper for cases where a qt dependency is not met.
dnl Output: If qt version is auto, set gemma_enable_qt to false. Else, exit.
AC_DEFUN([RAVEN_QT_FAIL],[
  if test "x$gemma_qt_want_version" = xauto && test "x$gemma_qt_force" != xyes; then
    if test "x$gemma_enable_qt" != xno; then
      AC_MSG_WARN([$1; gemma-qt frontend will not be built])
    fi
    gemma_enable_qt=no
    gemma_enable_qt_test=no
  else
    AC_MSG_ERROR([$1])
  fi
])
...
AC_DEFUN([RAVEN_QT_CHECK],[
  if test "x$gemma_enable_qt" != xno && test "x$gemma_qt_want_version" != xno; then
    true
    $1
  else
    true
...
AC_DEFUN([RAVEN_QT_INIT],[
  ...
  AC_ARG_WITH([gui],
    ...
    [build gemma-qt GUI (default=auto)])],
    [
     gemma_qt_want_version=$withval
     if test "x$gemma_qt_want_version" = xyes; then
       gemma_qt_force=yes
       gemma_qt_want_version=auto
     fi
    ],
    [gemma_qt_want_version=auto])
  ...
  AS_IF([test "x$with_gui" = xqt5_debug],
        [AS_CASE([$host],
                 ...
                 [qt_lib_suffix= ]); gemma_qt_want_version=qt5],
        [qt_lib_suffix= ])
...
dnl Outputs: gemma_enable_qt, gemma_enable_qt_dbus, gemma_enable_qt_test
AC_DEFUN([RAVEN_QT_CONFIGURE],[
  ...
  _RAVEN_QT_IS_STATIC
  if test "x$gemma_cv_static_qt" = xyes; then
  ...
  RAVEN_QT_CHECK([
    gemma_enable_qt=yes
    gemma_enable_qt_test=yes
    if test "x$have_qt_test" = xno; then
      gemma_enable_qt_test=no
    fi
    gemma_enable_qt_dbus=no
    if test "x$use_dbus" != xno && test "x$have_qt_dbus" = xyes; then
      gemma_enable_qt_dbus=yes
    fi
  ...
  if test x$gemma_enable_qt = xyes; then
    AC_MSG_RESULT([$gemma_enable_qt ($qt_lib_prefix)])
  else
    AC_MSG_RESULT([$gemma_enable_qt])
  fi
...
dnl Output: gemma_cv_static_qt=yes|no
AC_DEFUN([_RAVEN_QT_IS_STATIC],[
  AC_CACHE_CHECK(for static Qt, gemma_cv_static_qt,[
    AC_COMPILE_IFELSE([AC_LANG_PROGRAM([[
        #include <QtCore/qconfig.h>
        #ifndef QT_VERSION
        #  include <QtCore/qglobal.h>
      ...
      [gemma_cv_static_qt=yes],
      [gemma_cv_static_qt=no])
    ])
])
