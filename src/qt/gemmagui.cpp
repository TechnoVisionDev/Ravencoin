...
const std::string GemmaGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString GemmaGUI::DEFAULT_WALLET = "~Default";
...
static bool ThreadSafeMessageBox(GemmaGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

GemmaGUI::GemmaGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void GemmaGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Gemma address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and gemma: URIs)"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Gemma addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Gemma addresses"));
    ...
    openAction->setStatusTip(tr("Open a gemma: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Gemma command-line options").arg(tr(PACKAGE_NAME)));
...
void GemmaGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        if(IconsOnly) {
            ...
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/gemmacointext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* gemmaLabelLayout = new QVBoxLayout(toolbarWidget);
        gemmaLabelLayout->addWidget(labelToolbar);
        gemmaLabelLayout->addWidget(m_toolbar);
        gemmaLabelLayout->setDirection(QBoxLayout::TopToBottom);
        gemmaLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Gemma Market Price"));
...
void GemmaGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/rvntext")));
        m_toolbar->setMaximumWidth(65);
...
void GemmaGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool GemmaGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool GemmaGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void GemmaGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void GemmaGUI::setWalletActionsEnabled(bool enabled)
{
...
void GemmaGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void GemmaGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void GemmaGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void GemmaGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void GemmaGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void GemmaGUI::openClicked()
{
...
void GemmaGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void GemmaGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void GemmaGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void GemmaGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void GemmaGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void GemmaGUI::gotoVerifyMessageTab(QString addr)
{
...
void GemmaGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to Gemma network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void GemmaGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void GemmaGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void GemmaGUI::updateHeadersSyncProgressLabel()
{
...
void GemmaGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void GemmaGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Gemma"); // default title
...
void GemmaGUI::changeEvent(QEvent *e)
{
...
void GemmaGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void GemmaGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void GemmaGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
...
void GemmaGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void GemmaGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool GemmaGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool GemmaGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void GemmaGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void GemmaGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void GemmaGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void GemmaGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void GemmaGUI::detectShutdown()
{
...
void GemmaGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void GemmaGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void GemmaGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(GemmaGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void GemmaGUI::subscribeToCoreSignals()
{
...
void GemmaGUI::unsubscribeFromCoreSignals()
{
...
void GemmaGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void GemmaGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<GemmaUnits::Unit> units = GemmaUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const GemmaUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(GemmaUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(GemmaUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (GemmaUnits::Unit u : GemmaUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(GemmaUnits::name(u)), this);
...
{
    setText(GemmaUnits::name(newUnits));
}
...
void GemmaGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void GemmaGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "GemmaGUI::onPriceUnitChange: " + QString::number(newIndex);
