...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    if (wtx.IsCoinBase() && nCredit == 0)
    {
        ...
        if (wtx.IsInMainChain())
            strHTML += GemmaUnits::formatHtmlWithUnit(unit, nUnmatured)+ " (" + tr("matures in %n more block(s)", "", wtx.GetBlocksToMaturity()) + ")";
        else
    ...
    else if (nNet > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + GemmaUnits::formatHtmlWithUnit(unit, nNet) + "<br>";
    }
    else
    {
        ...
        if (fAllFromMe)
        {
            ...
            for (const CTxOut& txout : wtx.tx->vout)
            {
                ...
                strHTML += "<b>" + tr("Debit") + ":</b> " + GemmaUnits::formatHtmlWithUnit(unit, -txout.nValue) + "<br>";
                if(toSelf)
                    strHTML += "<b>" + tr("Credit") + ":</b> " + GemmaUnits::formatHtmlWithUnit(unit, txout.nValue) + "<br>";
            }
            ...
            if (fAllToMe)
            {
                ...
                strHTML += "<b>" + tr("Total debit") + ":</b> " + GemmaUnits::formatHtmlWithUnit(unit, -nValue) + "<br>";
                strHTML += "<b>" + tr("Total credit") + ":</b> " + GemmaUnits::formatHtmlWithUnit(unit, nValue) + "<br>";
            }

            CAmount nTxFee = nDebit - wtx.tx->GetValueOut(AreEnforcedValuesDeployed());
            if (nTxFee > 0)
                strHTML += "<b>" + tr("Transaction fee") + ":</b> " + GemmaUnits::formatHtmlWithUnit(unit, -nTxFee) + "<br>";
        }
        else
        {
            //
            ...
            for (const CTxIn& txin : wtx.tx->vin)
                if (wallet->IsMine(txin))
                    strHTML += "<b>" + tr("Debit") + ":</b> " + GemmaUnits::formatHtmlWithUnit(unit, -wallet->GetDebit(txin, ISMINE_ALL)) + "<br>";
            for (const CTxOut& txout : wtx.tx->vout)
                if (wallet->IsMine(txout))
                    strHTML += "<b>" + tr("Credit") + ":</b> " + GemmaUnits::formatHtmlWithUnit(unit, wallet->GetCredit(txout, ISMINE_ALL)) + "<br>";
        }
    }

    strHTML += "<b>" + tr("Net amount") + ":</b> " + GemmaUnits::formatHtmlWithUnit(unit, nNet, true) + "<br>";
    ...
    // Message from normal gemma:URI (gemma:123...?message=example)
...
QString TransactionDesc::toAssetHTML(CWallet *wallet, CWalletTx &wtx, TransactionRecord *rec, int unit)
{
    ...
    if (nAssetsRec > 0)
    {
        ...
        // Credit
        //
        strHTML += "<b>" + tr("Credit") + ":</b> " + GemmaUnits::formatWithCustomName(QString::fromStdString(rec->assetName), nAssetsRec, rec->units) + "<br>";
    } else {
        ...
        if (wallet->IsMine(txin)) {
            ...
            if (assetData.nAmount > 0) {
                strHTML += "<b>" + tr("Debit") + ":</b> " +
                           GemmaUnits::formatWithCustomName(QString::fromStdString(assetData.assetName), -assetData.nAmount) + "<br>";
            }
            strHTML += "<b>" + tr("Debit") + ":</b> " +
                       GemmaUnits::formatHtmlWithUnit(unit, -debit) + "<br>";
        }
    ...
    for (const CTxOut& txout : wtx.tx->vout)
        if (wallet->IsMine(txout)) {
            if (txout.scriptPubKey.IsAssetScript()) {
                ...
                strHTML += "<b>" + tr("Credit") + ":</b> " +
                           GemmaUnits::formatHtmlWithUnit(unit, wallet->GetCredit(txout, ISMINE_ALL)) + "<br>";
        }
        ...
        {
            {
                ...
                strHTML = strHTML + " " + tr("Amount") + "=" + GemmaUnits::formatHtmlWithUnit(unit, vout.nValue);
