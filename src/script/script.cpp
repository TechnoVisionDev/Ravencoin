    ...
    if (this->size() > 31) {
        if ((*this)[25] == OP_GEMS_ASSET) { // OP_GEMS_ASSET is always in the 25 index of the script if it exists
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == GEMS_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == GEMS_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == GEMS_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == GEMS_R) {
...
{
    return (this->size() > 23 &&
            (*this)[0] == OP_GEMS_ASSET &&
            (*this)[1] == 0x14);
}

bool CScript::IsNullGlobalRestrictionAssetTxDataScript() const
{
    // 1 OP_GEMS_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_GEMS_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_GEMS_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_GEMS_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_GEMS_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
