...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of GEMMA NOT allowed
        BOOST_CHECK(!IsAssetNameValid("GEMS", type));
        BOOST_CHECK(!IsAssetNameValid("RAVEN", type));
        BOOST_CHECK(!IsAssetNameValid("GEMMA", type));

        //- Versions of GEMMA ALLOWED
        BOOST_CHECK(IsAssetNameValid("RAVEN.COIN", type));
        BOOST_CHECK(IsAssetNameValid("RAVEN_COIN", type));
        BOOST_CHECK(IsAssetNameValid("GEMSSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERGEMS", type));
        ...
        BOOST_CHECK(IsAssetNameValid("SEGEMSOT", type));
