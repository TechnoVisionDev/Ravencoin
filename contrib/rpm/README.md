...
The RPM spec file provided here is for Gemma-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://gemma.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Gemma git repository](https://github.com/GemmaProject/Gemma) but are not part of
...
Currently only `Source10` is used, for the example `gemma.conf` file.
...
in the gemma git repository. At present only an SVG version of the Gemma
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the gemma
...
needed when 0.12.1 is released, a proper fix is already in the Gemma git
master branch.
...
idea to build Gemma Core against a static version of that library compiled
...
The `gemma.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `gemma-server` package. At this point, most RPM based
...
The `gemma-server` package should have SELinux support. How to properly do
...
Gemma-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/gemma/). He has been
packaging Gemma for Fedora at least since 2012.
...
4. I renamed the `gemma` package that contains the Qt GUI to `gemma-core` as
that appears to be how the general population refers to it, in contrast to
`gemma-xt` or `gemma-classic`. I wanted to make sure the general population
