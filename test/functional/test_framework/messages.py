...
"""
Gemma test framework primitive and message structures.
...
class CAddress:
    """
    Objects that map to gemmad objects, which can be serialized/deserialized
    """
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in gemmad
            if flags != 0:
                self.vin = deser_vector(f, CTxIn)
...
class MsgHeaders:
    ...
    def deserialize(self, f):
        # comment in gemmad indicates these should be deserialized as blocks
