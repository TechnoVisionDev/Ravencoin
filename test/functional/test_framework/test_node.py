...
"""Class for gemmad node under test"""
...
class TestNode:
    """A class for representing a gemmad node under test.
    ...
    def __init__(self, i, dirname, extra_args, rpchost, timewait, binary, stderr, mocktime, coverage_dir):
        ...
        if timewait:
            ...
            self.rpc_timeout = 60
        if binary is None:
            self.binary = os.getenv("RAVEND", "gemmad")
        else:
            self.binary = binary
        self.stderr = stderr
        ...
        self.cli = TestNodeCLI(os.getenv("RAVENCLI", "gemma-cli"), self.datadir)
    ...
    def __del__(self):
        # Ensure that we don't leave any gemmad processes lying around after
        # the test ends
    ...
    def start(self, extra_args=None, stderr=None):
        ...
        self.log.debug("gemmad started, waiting for RPC to come up")

    def wait_for_rpc_connection(self):
        """Sets up an RPC connection to the gemmad process. Returns False if unable to connect."""
        # Poll at a rate of four times per second
        poll_per_s = 4
        for _ in range(poll_per_s * self.rpc_timeout):
            assert self.process.poll() is None, "gemmad exited with status %i during initialization" % self.process.returncode
            try:
            ...
            except ValueError as e:  # cookie file not found and no rpcuser or rpcassword. gemmad still starting
                if "No RPC credentials" not in str(e):
                    raise
            time.sleep(1.0 / poll_per_s)
        raise AssertionError("Unable to connect to gemmad")
    ...
    def node_encrypt_wallet(self, passphrase):
        ...
        This causes gemmad to shutdown, so this method takes
...
class TestNodeCLI:
    """Interface to gemma-cli for an individual node"""
    ...
    def __call__(self, *args, input_data=None):
        # TestNodeCLI is callable with gemma-cli command-line args
    ...
    def send_cli(self, command, *args, **kwargs):
        """Run gemma-cli command. Deserializes returned string as python object."""

        pos_args = [str(arg) for arg in args]
        named_args = [str(key) + "=" + str(value) for (key, value) in kwargs.items()]
        assert not (pos_args and named_args), "Cannot use positional arguments and named arguments in the same gemma-cli call"
