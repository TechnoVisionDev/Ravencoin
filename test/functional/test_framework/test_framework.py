...
class GemmaTestFramework:
    """Base class for a gemma test script.

    Individual gemma test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def main(self):
        ...
        parser.add_option("--nocleanup", dest="nocleanup", default=False, action="store_true", help="Leave gemmads and test.* datadir on exit or error")
        parser.add_option("--noshutdown", dest="noshutdown", default=False, action="store_true", help="Don't stop gemmads after the test execution")
        ...
        parser.add_option("--srcdir", dest="srcdir", default=os.path.normpath(os.path.dirname(os.path.realpath(__file__)) + "/../../../src"), help="Source directory containing gemmad/gemma-cli (default: %default)")
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: gemmad's were not stopped and may still be running")
    ...
    def stop_node(self, i):
        """Stop a gemmad test node"""
        self.nodes[i].stop_node()
        self.nodes[i].wait_until_stopped()

    def stop_nodes(self):
        """Stop multiple gemmad test nodes"""
        for node in self.nodes:
    ...
    def assert_start_raises_init_error(self, i, extra_args=None, expected_msg=None):
        with tempfile.SpooledTemporaryFile(max_size=2 ** 16) as log_stderr:
            try:
                ...
                assert 'gemmad exited' in str(e)  # node must have shutdown
                ...
                if expected_msg is None:
                    assert_msg = "gemmad should have exited with an error"
                else:
                    assert_msg = "gemmad should have exited with expected error " + expected_msg
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as gemmad's debug.log with microprecision (so log files can be concatenated and sorted)
        ...
        if self.options.trace_rpc:
            rpc_logger = logging.getLogger("GemmaRPC")
    ...
    def _initialize_chain(self):
        ...
        if create_cache:
            ...
            # Create cache directories, run gemmads:
            for i in range(MAX_NODES):
                datadir = initialize_data_dir(self.options.cachedir, i)
                args = [os.getenv("RAVEND", "gemmad"), "-server", "-keypool=1", "-datadir=" + datadir, "-discover=0"]
                if i > 0:
        ...
        for i in range(self.num_nodes):
            ...
            initialize_data_dir(self.options.tmpdir, i)  # Overwrite port/rpcport in gemma.conf
