...
`test/functional/test_framework/test_framework.py:GemmaTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/gemma_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and gemmad
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_gemmad = config["components"].getboolean("ENABLE_RAVEND")
    if not (enable_wallet and enable_cli and enable_gemmad):
        print("No functional tests to run. Wallet, utils, and gemmad must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if gemmad is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "gemmad"]) is not None:
            print("%sWARNING!%s There is already a gemmad process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "RAVEND" not in os.environ:
        os.environ["RAVEND"] = build_dir + '/src/gemmad' + exeext
        os.environ["RAVENCLI"] = build_dir + '/src/gemma-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `gemma-cli help` (`rpc_interface.txt`).
